...
Regardless of our normalization approach, it is clear that much of the variability in our data arise from differences between the data sets. Unless we expect radically different cell types to be present in either data set, we must assume that non-biological factors are contributing to these differences. In the next section, we will go over a specific procedure for data integration, in which we account for these effects as part of our workflow.
...
Note, at this point we will not be doing anything with our `norm_seurat` object. So let's remove it to save space.
